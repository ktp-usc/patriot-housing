...
const defaultStoryContent = [
  "At Patriot Housing Project, we're working to change that narrative. Our mission is simple: provide housing, jobs, and hope for veterans who have fallen on hard times.",
...
const defaultMissionContent = [
  "Twelve years ago, Tri-County Veterans Support Network (TCVSN) was founded with one simple mission: to connect struggling Veterans in the Charleston area with the local resources they need to rebuild their lives.",
  "Over the years, one truth has remained constant: too many of our nation's heroes lack a safe, stable place to call home. Through partnerships with local community organizations, TCVSN has already helped more than 7,000 Veterans secure housing, employment, benefits, and critical support.",
  "But this challenge extends far beyond Charleston. That's why we launched the Patriot Housing Project, a nationwide initiative providing homes, job training, and hope for Veterans in communities across America.",
  "Now, we're calling on Patriots like you to stand up, step forward, and make a difference for the Veterans in your community. Together, we can ensure that every hero has a place to heal, rebuild, and thrive.",
];

function getParagraphs(blocks: PortableTextBlock[] | null | undefined, fallbacks: string[]) {
  const paragraphs = (blocks ?? [])
    .map((block) => block.children?.map((child) => child.text ?? "").join("").trim() ?? "")
    .filter((paragraph) => paragraph.length > 0);

  return paragraphs.length > 0 ? paragraphs : fallbacks;
}
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: homepage?.heroTitle || "Patriot Housing",
    description: homepage?.heroSubtitle || defaultHeroSubtitle,
  };
}

export default async function Home() {
  const homepage = await getHomepageContent();
  const storyParagraphs = getParagraphs(homepage?.storyContent, defaultStoryContent);
  const missionParagraphs = getParagraphs(homepage?.missionContent, defaultMissionContent);
  ...
  return (
    <div className="flex min-h-screen flex-col overflow-x-hidden bg-slate-50 text-slate-900">
      ...
      <main className="mx-auto flex w-full max-w-7xl flex-1 flex-col gap-6 px-4 py-6 sm:gap-8 sm:px-6 sm:py-8 md:px-10 md:py-12">
        <section className="relative flex min-h-72 items-center justify-center overflow-hidden rounded-2xl bg-[url('/images/backgroundImage.jpg')] bg-cover bg-center px-4 sm:min-h-80 sm:px-6 md:min-h-96 md:px-8">
          ...
          >
            ...
            <p className="theme-hero-copy mx-auto mt-4 max-w-3xl text-sm text-white sm:text-base md:mt-5 md:text-xl">
              {homepage?.heroSubtitle || defaultHeroSubtitle}
            </p>
          </div>
        </section>
        ...
        <section className="rounded-2xl bg-slate-50 py-12 shadow-xl ring-1 ring-white/10 backdrop-blur sm:py-16 md:py-20">
          <div className="mx-auto max-w-6xl px-4 sm:px-6">
            ...
            <div className="grid items-stretch gap-6 lg:grid-cols-2 lg:gap-10">
              <div className="flex h-full flex-col rounded-2xl bg-linear-to-br from-slate-900/90 to-slate-800/70 p-5 text-white shadow-xl ring-1 ring-white/10 backdrop-blur sm:p-6 md:p-8">
                <h3 className="text-xl font-semibold text-white sm:text-2xl">
                  Changing the Story for Veterans
                </h3>

                {storyParagraphs.map((paragraph, index) => (
                  <p key={`story-${index}`} className="mt-4 text-sm leading-relaxed text-slate-200 sm:text-base">
                    {paragraph}
                  </p>
                ))}
          ...
          <Carousel className="group mx-auto w-full max-w-5xl">
            <CarouselContent>
              {homepage?.carouselImages && homepage.carouselImages.length > 0 ? (
                homepage.carouselImages.map((image: HomepageCarouselImage, index: number) => (
                  <CarouselItem key={image._key || index}>
                    <div className="overflow-hidden rounded-2xl">
                      <Image
                        src={urlFor(image).url()}
                        alt={image.alt || `Patriot Housing carousel image ${index + 1}`}
                        className="block h-64 w-full object-cover sm:h-80 md:h-112"
                        width={800}
                        height={400}
                        priority={index === 0}
                      />
        ...
        <section className="rounded-2xl bg-slate-50 py-12 shadow-xl ring-1 ring-white/10 backdrop-blur sm:py-16 md:py-20">
          <div className="mx-auto max-w-5xl px-4 sm:px-6">
            ...
            <div className="rounded-2xl bg-linear-to-br from-slate-900/90 to-slate-800/70 p-5 shadow-xl ring-1 ring-slate-200 sm:p-6 md:p-12">
              <h3 className="text-lg font-bold text-slate-200 sm:text-xl">
                The Patriot Housing Project
              </h3>

              {missionParagraphs.map((paragraph, index) => (
                <p key={`mission-${index}`} className="mt-4 text-sm leading-relaxed text-slate-200 sm:text-base">
                  {paragraph}
                </p>
              ))}
